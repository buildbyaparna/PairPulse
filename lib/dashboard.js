...
function tradeReadiness(weightedScore, timeframes, activeSetup) {
  ...
  if (!activeVolumeCheck?.passed || !activeTrendCheck?.passed) {
    return "Avoid";
  }
    ...
    breakdown: [
      ...
      },
      {
        ...
        detail: Number.isFinite(Number(setup?.ratio))
          ? (setup?.qualifies ? `Reward to risk 1:${setup.ratio}` : `Raw reward to risk 1:${setup.ratio} | blocked by setup filters`)
