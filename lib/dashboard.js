...
function evaluateFrameContinuation(side, candles, timeframe) {
  ...
  const continuationReady = trendAligned && bos && choch && emaRetest && volumeConfirmed && notExhausted;
  ...
  return {
    ...
    detail: `${side === "long" ? "Breakout" : "Breakdown"} ${roundPriceLevel(breakoutLevel)} | RVOL ${relativeVolume.toFixed(2)}x`,
  };
}
...
function evaluateSetupPotential(side, entry, stopCandles, localCandles, structureCandles) {
  ...
  const ema20 = ema(executionCloses, 20).at(-1);
  const ema50 = ema(executionCloses, 50).at(-1);
  ...
  const distanceFromEmaAtr = Number.isFinite(ema20) ? Math.abs(entry - ema20) / atrSafe : Number.POSITIVE_INFINITY;
  ...
  if (side === "long") {
    ...
    const emaAligned = entry > ema20 && ema20 > ema50;
    const emaPositionOk = distanceFromEmaAtr <= 1.25 && candle.closeLocation >= 0.55;
    const volumeConfirmed = relativeVolume >= 1.15 && volumeTrend >= 1;
    const breakoutReady = !Number.isFinite(breakoutLevel) ||
      (breakoutLevel - entry) <= Math.max(atrSafe * 1.2, risk * 4, entry * 0.006);
    const exhausted =
      rsi15m >= 72 ||
      distanceFromEmaAtr > 1.45 ||
    ...
    const checks = [
      ...
      },
      {
        ...
        detail: `15m EMA20 ${formatSignedNumber((entry - ema20) / atrSafe)} ATR | EMA stack ${ema20 > ema50 ? "bullish" : "mixed"}`,
      },
      {
        label: "Volume",
        passed: volumeConfirmed,
  ...
  const emaAligned = entry < ema20 && ema20 < ema50;
  const emaPositionOk = distanceFromEmaAtr <= 1.25 && candle.closeLocation <= 0.45;
  const volumeConfirmed = relativeVolume >= 1.15 && volumeTrend >= 1;
  ...
  const checks = [
    ...
    },
    {
      ...
      passed: emaAligned && emaPositionOk,
      score: clamp(Math.round((emaAligned ? 60 : 18) + (emaPositionOk ? 40 : Math.max(0, 25 - distanceFromEmaAtr * 18))), 0, 100),
      detail: `15m EMA20 ${formatSignedNumber((entry - ema20) / atrSafe)} ATR | EMA stack ${ema20 < ema50 ? "bearish" : "mixed"}`,
    },
    {
      label: "Volume",
      passed: volumeConfirmed,
...
function isSetupWatchCandidate(row, side) {
  ...
  return Boolean(
    ...
    ((continuation?.score ?? 0) >= 55 || volumeCheck?.passed),
  );
}
...
function isContinuationWatchCandidate(row, side) {
  ...
  return Boolean(
    ...
    structurePresent &&
    (frame5?.volumeConfirmed || frame15?.volumeConfirmed || averageRvol >= 1.1),
  );
}
...
async function buildDashboardSnapshot() {
  ...
  const strictBearLeaders = rawRows
    ...
    .slice(0, 5);
  const bullWatchlist = rawRows
    .filter((row) => isSetupWatchCandidate(row, "long"))
    .sort((left, right) => {
      const leftVolumeCheck = Number(Boolean(findCheck(left.setups?.long?.checks, "Volume")?.passed));
      const rightVolumeCheck = Number(Boolean(findCheck(right.setups?.long?.checks, "Volume")?.passed));
      if (rightVolumeCheck !== leftVolumeCheck) {
        return rightVolumeCheck - leftVolumeCheck;
      }
      if ((right.continuations?.long?.score ?? 0) !== (left.continuations?.long?.score ?? 0)) {
        return (right.continuations?.long?.score ?? 0) - (left.continuations?.long?.score ?? 0);
      }
  ...
  const bearWatchlist = rawRows
    .filter((row) => isSetupWatchCandidate(row, "short"))
    .sort((left, right) => {
      const leftVolumeCheck = Number(Boolean(findCheck(left.setups?.short?.checks, "Volume")?.passed));
      const rightVolumeCheck = Number(Boolean(findCheck(right.setups?.short?.checks, "Volume")?.passed));
      if (rightVolumeCheck !== leftVolumeCheck) {
        return rightVolumeCheck - leftVolumeCheck;
      }
      if ((right.continuations?.short?.score ?? 0) !== (left.continuations?.short?.score ?? 0)) {
  ...
  const qualifiedBullLeaders = [
    ...strictBullLeaders.map((row) => decorateSetupLeader(row, "long", "confirmed")),
    ...bullWatchlist
      .filter((row) => !strictBullLeaders.some((candidate) => candidate.symbol === row.symbol))
      .slice(0, Math.max(0, 5 - strictBullLeaders.length))
      .map((row) => decorateSetupLeader(row, "long", "watch")),
  ];
  const qualifiedBearLeaders = [
    ...strictBearLeaders.map((row) => decorateSetupLeader(row, "short", "confirmed")),
    ...bearWatchlist
      .filter((row) => !strictBearLeaders.some((candidate) => candidate.symbol === row.symbol))
      .slice(0, Math.max(0, 5 - strictBearLeaders.length))
      .map((row) => decorateSetupLeader(row, "short", "watch")),
  ];
