...
function stopQualityScore(entry, setup) {
  const risk = Number(setup?.risk);
  const price = Number(entry);

  if (!Number.isFinite(risk) || risk <= 0 || !Number.isFinite(price) || price <= 0) {
    return 0;
  }
  ...
  if (riskPct < 0.15) {
    return Math.round(15 + ((riskPct - 0.05) / 0.10) * 25);
  }
  if (riskPct < 0.4) {
    return Math.round(40 + ((riskPct - 0.15) / 0.25) * 30);
  }
  if (riskPct < 1.2) {
    return Math.round(70 + ((riskPct - 0.4) / 0.8) * 30);
  }
  if (riskPct < 2.5) {
    return Math.round(100 - ((riskPct - 1.2) / 1.3) * 20);
  }
  if (riskPct < 5) {
    return Math.round(80 - ((riskPct - 2.5) / 2.5) * 30);
  }

  return 45;
}

function calculateSetupScore(biasScore, readiness, setup, price) {
  ...
  const stopValue = stopQualityScore(price, setup);
  ...
  return {
    ...
    breakdown: [
      ...
      {
        label: "15m stop quality",
        score: stopValue,
        detail: Number.isFinite(Number(setup?.risk)) && Number.isFinite(Number(price)) && Number(price) > 0
          ? `Stop distance ${((Number(setup.risk) / Number(price)) * 100).toFixed(2)}%`
          : "No valid stop distance",
      },
    ],
  };
}
...
async function buildRow(ticker) {
  ...
  const setupScore = calculateSetupScore(weightedScore, readiness, activeSetup, price);
