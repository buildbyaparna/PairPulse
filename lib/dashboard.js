...
function tradeReadiness(weightedScore, timeframes) {
...
async function buildRow(ticker) {
  ...
  return {
    ...
    price: ticker?.close ?? ticker?.mark_price ?? timeframeResults["15m"].close ?? null,
...
async function buildDashboardSnapshot() {
  ...
  return {
    ...
    majors,
    leaders: {
      bulls: rows.slice(0, 5),
      bears: [...rows].filter((row) => row.score < 0).slice(-5).reverse(),
    },
    allRows: rows,
  };
}
